...
(struct reference [location] #:transparent)
...
(define (interpret expr functions)
  (define (interpret/a expr env store)
    (match expr
      ...
      [(call fn arg)
       (define-values (value-arg store+)
         (if (and (string? arg) (defined? arg env))
             (values (reference (lookup arg env)) store)
             (interpret/a arg env store)))  
  ...
  (define (apply fvalue value-of-arg store)
    (define-values (loc store+)
      (if (reference? value-of-arg)
          (values (reference-location value-of-arg) store)
          (alloc store value-of-arg)))
    (define env+ (add (fun-value-parameter fvalue) loc
                      (fun-value-env fvalue)))
    (interpret/a (fun-value-body fvalue) env+ store+))
  
  (let-values ([(v s) (interpret/a expr empty empty-store)])
    v))
...
(module+ test
  (require (only-in "../test/L5/assign.rkt" assign-16-in assign-16-out))
  (require "../test/L5/test-L5.rkt")
  (require "../test/L5/test-pass-by-ref.rkt")
  (define (exclude-assign-16-interp p)
    (match p
      [(program _ _) (interpret-prog p)]
      [_(if  (equal? assign-16-in p)
             assign-16-out
             (interpret p empty))]))
  (test-L5 exclude-assign-16-interp)
  (test-reference (λ (p) (interpret p empty))))
